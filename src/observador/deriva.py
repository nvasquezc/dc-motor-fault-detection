...
def fig_deriva(df_res, salida="docs/fig04_deriva.pdf"):
    ...
    fig = plt.figure(figsize=(COL_DOBLE, 122 * MM))
    gs = fig.add_gridspec(
        3, 2, width_ratios=[2.15, 1], height_ratios=[2.1, 0.8, 1.4],
        hspace=0.62, wspace=0.42,
        left=0.085, right=0.895, top=0.945, bottom=0.075,
    )
    ...
    ax.plot(t, d, "o", color=AZUL, mec=TINTA, mew=0.4, ms=2.8,
    ...
    axr.plot(t, fit["resid"], "o", color=AZUL, mec=TINTA, mew=0.3,
             ms=2.4, ls="none", zorder=4)
    ...
    # ---------- (c) Velocidad ----------
    axw.plot(t, w, "-s", color=VERDE, mec=TINTA, mew=0.35, ms=2.6,
             lw=0.9, zorder=4)
    ...
    axd.plot(t, dev, "-o", color=AZUL, mec=TINTA, mew=0.3,
             ms=2.4, lw=0.8, zorder=4)
    ...
    axd.set_xlabel("Tiempo de operación\ncontinua  (min)", fontsize=7.5,
                   linespacing=1.3)
    ...
    axc.text(0.02, 0.52, "Parámetros del ajuste", fontsize=7.2,
             fontweight="bold", va="center")
    axc.plot([0.02, 0.98], [0.43, 0.43], color=TINTA, lw=0.7,
             clip_on=False)
    ...
    axc.plot([0.02, 0.98], [y + 0.07, y + 0.07], color=TINTA,
             lw=0.7, clip_on=False)
    axc.text(0.06, y - 0.04,
             f"Estabilización ($3\\sigma$):  {t_3s:.0f} min",
             fontsize=6.9, color=ROJO, va="center")

    # ---------- (f) Normalidad del ruido ----------
    if MOSTRAR_HISTOGRAMA:
        axh.hist(res_reg, bins=18, density=True, color=AZUL,
                 alpha=0.55, edgecolor=TINTA, lw=0.4, zorder=3)
        ...
        axh.set_xlabel("Residuo en régimen  (%)", fontsize=7.5)
    ...
    _panel(ax, "(a)", dx=-0.095)
    _panel(axr, "(b)", dx=-0.095, dy=1.28)
    _panel(axw, "(c)", dx=-0.095)
    _panel(axd, "(d)", dx=-0.30)
    if MOSTRAR_HISTOGRAMA:
        _panel(axh, "(e)", dx=-0.30)
